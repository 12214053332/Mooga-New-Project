...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if (isset($_POST['type'], $_POST['short']) && in_array($_POST['type'], ['project', 'offer']) && in_array($_POST['short'], ['asc', 'desc'])) {
        ...
        $db->getInstance();
        $result = '';
        if(isset($_POST['dataType'])&&$_POST['dataType']=='VIEWS'){
            foreach ($report->views($_POST['type'], "", "ORDER BY `count` " . $_POST['short']) as $item) {
                $result .= '
        <tr>
            ...
            ' . (($_POST['type'] == 'project') ? '<td class="text-center"><p style=" width: 250px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">' . $item->name . '</p></td>' : '') . '
             <td class="text-center">'.(($_POST['type'] == 'project')?'<p style=" width: 250px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">' . $item->description . '</p>':'<p>'.$item->description .'</p>').'</td>
        </tr>
        ';
            }
            header('Content-Type: application/json');
