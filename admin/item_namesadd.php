...
$item_names_add = NULL; // Initialize page object first

class citem_names_add extends citem_names {
	...
	// Page object name
	var $PageObjName = 'item_names_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("item_nameslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "item_namesview.php")
	...
	function LoadDefaultValues() {
		...
		$this->kind_id->CurrentValue = NULL;
	...
	function RenderRow() {
		...
		$this->kind_id->ViewCustomAttributes = "";
			...
			$this->kind_id->EditValue = ew_HtmlEncode($this->kind_id->CurrentValue);
			$this->kind_id->PlaceHolder = ew_RemoveHtml($this->kind_id->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// kind_id
		$this->kind_id->SetDbValueDef($rsnew, $this->kind_id->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($item_names_add)) $item_names_add = new citem_names_add();

// Page init
$item_names_add->Page_Init();

// Page main
$item_names_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$item_names_add->Page_Render();
?>
...
var CurrentForm = fitem_namesadd = new ew_Form("fitem_namesadd", "add");

// Validate form
fitem_namesadd.Validate = function() {
...
fitem_namesadd.Form_CustomValidate = 
...
fitem_namesadd.ValidateRequired = true;
<?php } else { ?>
fitem_namesadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fitem_namesadd.Lists["x_brand_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":["x_type_id"],"FilterFields":[],"Options":[],"Template":""};
fitem_namesadd.Lists["x_type_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":["x_brand_id"],"ChildFields":[],"FilterFields":["x_brand_id"],"Options":[],"Template":""};
...
<?php $item_names_add->ShowPageHeader(); ?>
<?php
$item_names_add->ShowMessage();
?>
<form name="fitem_namesadd" id="fitem_namesadd" class="<?php echo $item_names_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($item_names_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $item_names_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fitem_namesadd.CreateAutoSuggest({"id":"x_brand_id","forceSelect":false});
</script>
</span>
...
<script type="text/javascript">
fitem_namesadd.CreateAutoSuggest({"id":"x_type_id","forceSelect":false});
</script>
</span>
...
<?php if ($item_names->kind_id->Visible) { // kind_id ?>
	<div id="r_kind_id" class="form-group">
		<label id="elh_item_names_kind_id" class="col-sm-2 control-label ewLabel"><?php echo $item_names->kind_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $item_names->kind_id->CellAttributes() ?>>
<span id="el_item_names_kind_id">
<input type="text" data-table="item_names" data-field="x_kind_id" name="x_kind_id" id="x_kind_id" size="30" placeholder="<?php echo ew_HtmlEncode($item_names->kind_id->getPlaceHolder()) ?>" value="<?php echo $item_names->kind_id->EditValue ?>"<?php echo $item_names->kind_id->EditAttributes() ?>>
</span>
<?php echo $item_names->kind_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $item_names_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
