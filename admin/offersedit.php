...
$offers_edit = NULL; // Initialize page object first

class coffers_edit extends coffers {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// offer_type_filed
			$this->offer_type_filed->SetDbValueDef($rsnew, $this->offer_type_filed->CurrentValue, NULL, $this->offer_type_filed->ReadOnly);
			...
			// item_brand
			$this->item_brand->SetDbValueDef($rsnew, $this->item_brand->CurrentValue, NULL, $this->item_brand->ReadOnly);

			// min_qty
			$this->min_qty->SetDbValueDef($rsnew, $this->min_qty->CurrentValue, NULL, $this->min_qty->ReadOnly);
			...
			// states
			$this->states->SetDbValueDef($rsnew, $this->states->CurrentValue, NULL, $this->states->ReadOnly);
			...
			// contact_type
			$this->contact_type->SetDbValueDef($rsnew, $this->contact_type->CurrentValue, NULL, $this->contact_type->ReadOnly);
			...
			// verified_code
			$this->verified_code->SetDbValueDef($rsnew, $this->verified_code->CurrentValue, NULL, $this->verified_code->ReadOnly);

			// pending
			$this->pending->SetDbValueDef($rsnew, $this->pending->CurrentValue, NULL, $this->pending->ReadOnly);

			// country_1
			$this->country_1->SetDbValueDef($rsnew, $this->country_1->CurrentValue, NULL, $this->country_1->ReadOnly);

			// num_send_sms
			$this->num_send_sms->SetDbValueDef($rsnew, $this->num_send_sms->CurrentValue, NULL, $this->num_send_sms->ReadOnly);

			// num_send_email
			$this->num_send_email->SetDbValueDef($rsnew, $this->num_send_email->CurrentValue, NULL, $this->num_send_email->ReadOnly);

			// done
			$this->done->SetDbValueDef($rsnew, $this->done->CurrentValue, 0, $this->done->ReadOnly);
...
// Create page object
if (!isset($offers_edit)) $offers_edit = new coffers_edit();

// Page init
$offers_edit->Page_Init();

// Page main
$offers_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$offers_edit->Page_Render();
?>
...
var CurrentForm = foffersedit = new ew_Form("foffersedit", "edit");

// Validate form
foffersedit.Validate = function() {
...
foffersedit.Form_CustomValidate = 
...
foffersedit.ValidateRequired = true;
<?php } else { ?>
foffersedit.ValidateRequired = false; 
<?php } ?>
...
<?php $offers_edit->ShowPageHeader(); ?>
<?php
$offers_edit->ShowMessage();
?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($offers_edit->Pager)) $offers_edit->Pager = new cPrevNextPager($offers_edit->StartRec, $offers_edit->DisplayRecs, $offers_edit->TotalRecs) ?>
<?php if ($offers_edit->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($offers_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $offers_edit->PageUrl() ?>start=<?php echo $offers_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($offers_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $offers_edit->PageUrl() ?>start=<?php echo $offers_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $offers_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($offers_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $offers_edit->PageUrl() ?>start=<?php echo $offers_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($offers_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $offers_edit->PageUrl() ?>start=<?php echo $offers_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $offers_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<form name="foffersedit" id="foffersedit" class="<?php echo $offers_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($offers_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $offers_edit->Token ?>">
<?php } ?>
...
<?php if ($offers->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_offers_id" class="col-sm-2 control-label ewLabel"><?php echo $offers->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $offers->id->CellAttributes() ?>>
<span id="el_offers_id">
<span<?php echo $offers->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $offers->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="offers" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($offers->id->CurrentValue) ?>">
<?php echo $offers->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $offers_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($offers_edit->Pager)) $offers_edit->Pager = new cPrevNextPager($offers_edit->StartRec, $offers_edit->DisplayRecs, $offers_edit->TotalRecs) ?>
<?php if ($offers_edit->Pager->RecordCount > 0) { ?>
