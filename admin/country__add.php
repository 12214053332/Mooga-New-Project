...
$country___add = NULL; // Initialize page object first

class ccountry___add extends ccountry__ {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "country__view.php")
	...
	function AddRow($rsold = NULL) {
		...
		// iso
		$this->iso->SetDbValueDef($rsnew, $this->iso->CurrentValue, "", FALSE);

		// nicename
		$this->nicename->SetDbValueDef($rsnew, $this->nicename->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($country___add)) $country___add = new ccountry___add();

// Page init
$country___add->Page_Init();

// Page main
$country___add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$country___add->Page_Render();
?>
...
var CurrentForm = fcountry__add = new ew_Form("fcountry__add", "add");

// Validate form
fcountry__add.Validate = function() {
...
fcountry__add.Form_CustomValidate = 
...
fcountry__add.ValidateRequired = true;
<?php } else { ?>
fcountry__add.ValidateRequired = false; 
<?php } ?>
...
<?php $country___add->ShowPageHeader(); ?>
<?php
$country___add->ShowMessage();
?>
<form name="fcountry__add" id="fcountry__add" class="<?php echo $country___add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($country___add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $country___add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $country___add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
