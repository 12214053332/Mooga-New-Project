...
class cemployees extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('employees', 'employees', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// report_level
		$this->report_level = new cField('employees', 'employees', 'x_report_level', 'report_level', '`report_level`', '`report_level`', 3, -1, FALSE, '`report_level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->report_level->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['report_level'] = &$this->report_level;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->report_level->EditValue = $this->report_level->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->report_level->Exportable) $Doc->ExportCaption($this->report_level);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->username->Exportable) $Doc->ExportCaption($this->username);
					if ($this->password->Exportable) $Doc->ExportCaption($this->password);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->report_level->Exportable) $Doc->ExportField($this->report_level);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->username->Exportable) $Doc->ExportField($this->username);
						if ($this->password->Exportable) $Doc->ExportField($this->password);
	...
	function UserIDFilter($userid) {
		...
		$sParentUserIDFilter = '`id` IN (SELECT `id` FROM ' . "`employees`" . ' WHERE `UserLevel` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID) . ')';
