...
//
// Table class for articles
//
class carticles extends cTable {
	var $id;
	var $category_id;
	var $name;
	var $description;
	...
	function __construct() {
		...
		// id
		$this->id = new cField('articles', 'articles', 'x_id', 'id', '`id`', '`id`', 20, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// category_id
		$this->category_id = new cField('articles', 'articles', 'x_category_id', 'category_id', '`category_id`', '`category_id`', 20, -1, FALSE, '`category_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->category_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['category_id'] = &$this->category_id;

		// name
		$this->name = new cField('articles', 'articles', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['name'] = &$this->name;

		// description
		$this->description = new cField('articles', 'articles', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['description'] = &$this->description;

		// picpath
		$this->picpath = new cField('articles', 'articles', 'x_picpath', 'picpath', '`picpath`', '`picpath`', 200, -1, TRUE, '`picpath`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->fields['picpath'] = &$this->picpath;

		// article_date
		$this->article_date = new cField('articles', 'articles', 'x_article_date', 'article_date', '`article_date`', 'DATE_FORMAT(`article_date`, \'%Y/%m/%d\')', 135, 5, FALSE, '`article_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->article_date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['article_date'] = &$this->article_date;

		// author_id
		$this->author_id = new cField('articles', 'articles', 'x_author_id', 'author_id', '`author_id`', '`author_id`', 20, -1, FALSE, '`author_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->author_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['author_id'] = &$this->author_id;

		// views
		$this->views = new cField('articles', 'articles', 'x_views', 'views', '`views`', '`views`', 3, -1, FALSE, '`views`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->views->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['views'] = &$this->views;

		// public
		$this->public = new cField('articles', 'articles', 'x_public', 'public', '`public`', '`public`', 201, -1, FALSE, '`public`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fields['public'] = &$this->public;

		// public_title
		$this->public_title = new cField('articles', 'articles', 'x_public_title', 'public_title', '`public_title`', '`public_title`', 200, -1, FALSE, '`public_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`articles`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("articlesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("articlesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "articlesadd.php?" . $this->UrlParm($parm);
		else
			$url = "articlesadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("articlesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("articlesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("articlesdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->article_date->EditValue = ew_FormatDateTime($this->article_date->CurrentValue, 5);
		...
		$this->public->EditValue = $this->public->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->article_date->Exportable) $Doc->ExportCaption($this->article_date);
					if ($this->author_id->Exportable) $Doc->ExportCaption($this->author_id);
					if ($this->views->Exportable) $Doc->ExportCaption($this->views);
					if ($this->public->Exportable) $Doc->ExportCaption($this->public);
					if ($this->public_title->Exportable) $Doc->ExportCaption($this->public_title);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->category_id->Exportable) $Doc->ExportCaption($this->category_id);
					if ($this->name->Exportable) $Doc->ExportCaption($this->name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->article_date->Exportable) $Doc->ExportField($this->article_date);
						if ($this->author_id->Exportable) $Doc->ExportField($this->author_id);
						if ($this->views->Exportable) $Doc->ExportField($this->views);
						if ($this->public->Exportable) $Doc->ExportField($this->public);
						if ($this->public_title->Exportable) $Doc->ExportField($this->public_title);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->category_id->Exportable) $Doc->ExportField($this->category_id);
						if ($this->name->Exportable) $Doc->ExportField($this->name);
