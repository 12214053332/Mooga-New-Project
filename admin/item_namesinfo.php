...
//
// Table class for item_names
//
class citem_names extends cTable {
	var $id;
	var $brand_id;
	var $type_id;
	var $name;
	...
	function __construct() {
		...
		$this->TableVar = 'item_names';
		$this->TableName = 'item_names';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`item_names`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('item_names', 'item_names', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// brand_id
		$this->brand_id = new cField('item_names', 'item_names', 'x_brand_id', 'brand_id', '`brand_id`', '`brand_id`', 3, -1, FALSE, '`brand_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->brand_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['brand_id'] = &$this->brand_id;

		// type_id
		$this->type_id = new cField('item_names', 'item_names', 'x_type_id', 'type_id', '`type_id`', '`type_id`', 3, -1, FALSE, '`type_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->type_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['type_id'] = &$this->type_id;

		// name
		$this->name = new cField('item_names', 'item_names', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['name'] = &$this->name;

		// approved
		$this->approved = new cField('item_names', 'item_names', 'x_approved', 'approved', '`approved`', '`approved`', 3, -1, FALSE, '`approved`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->approved->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['approved'] = &$this->approved;

		// kind_id
		$this->kind_id = new cField('item_names', 'item_names', 'x_kind_id', 'kind_id', '`kind_id`', '`kind_id`', 3, -1, FALSE, '`kind_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`item_names`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("item_namesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("item_namesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "item_namesadd.php?" . $this->UrlParm($parm);
		else
			$url = "item_namesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("item_namesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("item_namesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("item_namesdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->brand_id->Exportable) $Doc->ExportCaption($this->brand_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->brand_id->Exportable) $Doc->ExportField($this->brand_id);
