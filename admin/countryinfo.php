...
class ccountry extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('country', 'country', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// iso
		$this->iso = new cField('country', 'country', 'x_iso', 'iso', '`iso`', '`iso`', 200, -1, FALSE, '`iso`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['iso'] = &$this->iso;

		// nicename
		$this->nicename = new cField('country', 'country', 'x_nicename', 'nicename', '`nicename`', '`nicename`', 200, -1, FALSE, '`nicename`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nicename'] = &$this->nicename;

		// name
		$this->name = new cField('country', 'country', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['name'] = &$this->name;

		// iso3
		$this->iso3 = new cField('country', 'country', 'x_iso3', 'iso3', '`iso3`', '`iso3`', 200, -1, FALSE, '`iso3`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['iso3'] = &$this->iso3;

		// numcode
		$this->numcode = new cField('country', 'country', 'x_numcode', 'numcode', '`numcode`', '`numcode`', 2, -1, FALSE, '`numcode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->numcode->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numcode'] = &$this->numcode;

		// code
		$this->code = new cField('country', 'country', 'x_code', 'code', '`code`', '`code`', 3, -1, FALSE, '`code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->code->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['code'] = &$this->code;

		// arab_name
		$this->arab_name = new cField('country', 'country', 'x_arab_name', 'arab_name', '`arab_name`', '`arab_name`', 200, -1, FALSE, '`arab_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['arab_name'] = &$this->arab_name;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->arab_name->EditValue = $this->arab_name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->arab_name->Exportable) $Doc->ExportCaption($this->arab_name);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->iso->Exportable) $Doc->ExportCaption($this->iso);
					if ($this->nicename->Exportable) $Doc->ExportCaption($this->nicename);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->arab_name->Exportable) $Doc->ExportField($this->arab_name);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->iso->Exportable) $Doc->ExportField($this->iso);
						if ($this->nicename->Exportable) $Doc->ExportField($this->nicename);
