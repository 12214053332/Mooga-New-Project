...
class ccountry_edit extends ccountry {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nicename
			$this->nicename->SetDbValueDef($rsnew, $this->nicename->CurrentValue, NULL, $this->nicename->ReadOnly);
...
fcountryedit.ValidateRequired = true;
<?php } else { ?>
fcountryedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!isset($country_edit->Pager)) $country_edit->Pager = new cPrevNextPager($country_edit->StartRec, $country_edit->DisplayRecs, $country_edit->TotalRecs) ?>
<?php if ($country_edit->Pager->RecordCount > 0) { ?>
<div class="ewPager">
...
<?php if ($country->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_country_id" class="col-sm-2 control-label ewLabel"><?php echo $country->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $country->id->CellAttributes() ?>>
<span id="el_country_id">
<span<?php echo $country->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $country->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="country" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($country->id->CurrentValue) ?>">
<?php echo $country->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
