...
class cemployees_list extends cemployees {
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->report_level->AdvancedSearch->ToJSON(), ","); // Field report_level
		if ($this->BasicSearch->Keyword <> "") {
	...
	function RestoreFilterList() {
		...
		// Field report_level
		$this->report_level->AdvancedSearch->SearchValue = @$filter["x_report_level"];
		$this->report_level->AdvancedSearch->SearchOperator = @$filter["z_report_level"];
		$this->report_level->AdvancedSearch->SearchCondition = @$filter["v_report_level"];
		$this->report_level->AdvancedSearch->SearchValue2 = @$filter["y_report_level"];
		$this->report_level->AdvancedSearch->SearchOperator2 = @$filter["w_report_level"];
		$this->report_level->AdvancedSearch->Save();
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->report_level); // report_level
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"femployeeslistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
femployeeslist.ValidateRequired = true;
<?php } else { ?>
femployeeslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
femployeeslist.Lists["x_UserLevel"] = {"LinkField":"x_userlevelid","Ajax":true,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php
	...
	if ($bSelectLimit) {
		if ($employees_list->TotalRecs <= 0)
			$employees_list->TotalRecs = $employees->SelectRecordCount();
	} else {
	...
	if ($employees->CurrentAction == "" && $employees_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$employees_list->setWarningMessage($Language->Phrase("NoPermission"));
...
<form name="femployeeslistsrch" id="femployeeslistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
...
<?php if (!isset($employees_list->Pager)) $employees_list->Pager = new cPrevNextPager($employees_list->StartRec, $employees_list->DisplayRecs, $employees_list->TotalRecs) ?>
...
<?php } ?>
<input type="hidden" name="t" value="employees">
<div id="gmp_employees" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($employees_list->TotalRecs > 0) { ?>
...
<?php if ($employees->id->Visible) { // id ?>
	<?php if ($employees->SortUrl($employees->id) == "") { ?>
		<th data-name="id"><div id="elh_employees_id" class="employees_id"><div class="ewTableHeaderCaption"><?php echo $employees->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $employees->SortUrl($employees->id) ?>',1);"><div id="elh_employees_id" class="employees_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $employees->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($employees->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($employees->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($employees->username->Visible) { // username ?>
	<?php if ($employees->SortUrl($employees->username) == "") { ?>
		<th data-name="username"><div id="elh_employees_username" class="employees_username"><div class="ewTableHeaderCaption"><?php echo $employees->username->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="username"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $employees->SortUrl($employees->username) ?>',1);"><div id="elh_employees_username" class="employees_username">
...
<?php if ($employees->password->Visible) { // password ?>
	<?php if ($employees->SortUrl($employees->password) == "") { ?>
		<th data-name="password"><div id="elh_employees_password" class="employees_password"><div class="ewTableHeaderCaption"><?php echo $employees->password->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="password"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $employees->SortUrl($employees->password) ?>',1);"><div id="elh_employees_password" class="employees_password">
...
<?php if ($employees->UserLevel->Visible) { // UserLevel ?>
	<?php if ($employees->SortUrl($employees->UserLevel) == "") { ?>
		<th data-name="UserLevel"><div id="elh_employees_UserLevel" class="employees_UserLevel"><div class="ewTableHeaderCaption"><?php echo $employees->UserLevel->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="UserLevel"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $employees->SortUrl($employees->UserLevel) ?>',1);"><div id="elh_employees_UserLevel" class="employees_UserLevel">
...
<?php if ($employees->report_level->Visible) { // report_level ?>
	<?php if ($employees->SortUrl($employees->report_level) == "") { ?>
		<th data-name="report_level"><div id="elh_employees_report_level" class="employees_report_level"><div class="ewTableHeaderCaption"><?php echo $employees->report_level->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="report_level"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $employees->SortUrl($employees->report_level) ?>',1);"><div id="elh_employees_report_level" class="employees_report_level">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $employees->report_level->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($employees->report_level->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($employees->report_level->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<?php echo $employees->id->ListViewValue() ?></span>
</span>
<a id="<?php echo $employees_list->PageObjName . "_row_" . $employees_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($employees->username->Visible) { // username ?>
...
<?php echo $employees->UserLevel->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($employees->report_level->Visible) { // report_level ?>
		<td data-name="report_level"<?php echo $employees->report_level->CellAttributes() ?>>
<span id="el<?php echo $employees_list->RowCnt ?>_employees_report_level" class="employees_report_level">
<span<?php echo $employees->report_level->ViewAttributes() ?>>
<?php echo $employees->report_level->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
