	/*function*/
		  ...
		  function add_project() {
				...
				$('.btn.orange-btn').attr('disabled','disabled');
                $.ajax({
                    type: 'post',
					...
					processData:false, 
                    success: function (data) {
						$('.btn.orange-btn').removeAttr('disabled');
						if (IsJsonString(data))
						 {
		  ...
		  function add_offer() {
                ...
                var offer_type_filed = $('#offer_type_filed').chosen().val();
				offer_type_filed= JSON.stringify(offer_type_filed);
				...
				state= JSON.stringify(state);
				  ...
				  var cities = $('#cities').chosen().val();
				cities= JSON.stringify(cities);
				 

                
				 var formdata= new FormData(document.getElementById("addoffer-form"));
				 
				//formdata.append('offer_type_filed', offer_type_filed);
			  ...
			  $('.btn.orange-btn').attr('disabled','disabled');
                $.ajax({
                    type: 'post',
                    url:/****/'?page=_offeraction&action=addoffer',
                    data: formdata,
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
					processData:false, 
                    success: function (data) {
						...
						if (IsJsonString(data))
						 {
							   var id = JSON.parse(data);
							   window.location.assign('?page=verifie_offer&offer_id='+id.id);
						 }
						
						 else {
                          $('#addoffer-response').html(data);
                        }
						ajaxbalance();  
						/*  $("input[type='text']").val('');
						  $("input[type='file']").val('');
	 ...
	 var formdata= new FormData(document.getElementById("change_offer-form"));
                $.ajax({
                    type: 'post',
                    url:/****/'?page=_offeraction&action=change_offer',
                    data: formdata,
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
					processData:false, 
                    success: function (data) {
                          ...
                          $('#change_offer-response').html(data);
	 ...
	 var formdata= new FormData(document.getElementById("change_project-form"));
                $.ajax({
                    type: 'post',
                    url:/****/'?page=_projectaction&action=change_project',
                    data: formdata,
					contentType: false,       // The content type used when sending data to the server.
					cache: false,             // To unable request pages to be cached
					processData:false, 
                    success: function (data) {
                          ...
                          $('#change_project-response').html(data);
		 ...
		 function addopportunity() {
		  ...
		  function allofferssearch() {
			...
			/*	 var country = $('#country').chosen().val();
                ...
                $.ajax({
                    type: 'post',
                    url: /****/'?page=_searchaction&action=alloffers',
                    data: $('#alloffers-form').serialize(),//+parameter,
                    success: function (data) {
						//alert(data);
	                 $('#SearchSpinner').removeClass('spinner');						
						$('#alloffers-response').html(data);
						
						$(window).scrollTop($('#alloffers-response').offset().top);
			...
			function lazy_allofferssearch() {
				...
				*/
			   var lastofferid=$('#offerid').data('offerid');
			   	
				lastofferid="lastofferid="+lastofferid;
				
				$( "#offerid" ).remove();
				var parameter="&"+lastofferid;
				//alert(parameter);
                $.ajax({
                    type: 'post',
                    url: /****/'?page=_searchaction&action=alloffers',
					...
					//	alert($('#projectid').data('projectid'));
						...
						if (!data.trim()){
						 ...
						 $('#alloffers-response').append("<h3 class='full-lines sm-font' id='no-result-found'>لا توجد نتائج</h3>");		
						}
						else
						{
					
				         $('#alloffers-response').append(data);
						//after_lazy_offers();
                         showoffernumber_all();
		  ...
		  function myofferssearch() {
			...
			/*	 var country = $('#country').chosen().val();
                ...
                $.ajax({
                    type: 'post',
                    url: /****/'?page=_searchaction&action=myoffers',
                    data: $('#myoffers-form').serialize(),//+parameter,
                    success: function (data) {
						//alert(data);
	                 $('#SearchSpinner').removeClass('spinner');						
						$('#alloffers-response').html(data);
		...
		function lazy_myprojectssearch() {
		  ...
		  function allprojectssearch() {
			...
			function lazy_allarticlessearch(){
				var lastarticleid=$('#articleid').data('articleid');
				lastarticleid="lastarticleid="+lastarticleid;
				var catid=$('#catid').data('catid');
				catid="catid="+catid;
				$( "#articleid" ).remove();
				$( "#catid" ).remove();
				var parameter="&"+lastarticleid+"&"+catid;
				console.log(parameter);
				$.ajax({
					type: 'post',
					url: /****/'?page=_searchaction&action=allarticles',
					data: parameter,
					success: function (data) {
						//	alert($('#projectid').data('projectid'));
						//alert(data);
						...
						if (!data.trim()){
							$('#no-result-found').remove();
							$('#allarticles-response').append("<h3 class='full-lines sm-font' id='no-result-found'>لا توجد نتائج</h3>");
						}
						else
						{
							$('#allarticles-response').append(data);
		...
		function check_showprojectphone(project_id,user_id,e) {
				...
				function showofferphone(offer_id1,user_id1,e) {

                $.ajax({
                    type: 'post',
                    url:/****/'?page=_offeraction&action=showphone',
                    data: {offer_id:offer_id1,user_id:user_id1},
                    success: function (data) {
						...
						$(e).unbind( "click" );
						$(e).removeClass( "offer-show-phone" );
						}
						ajaxbalance();
						//$(e).append(data);
						//$(e).removeclass(data);
		...
		function check_showofferphone(offer_id1,user_id,e) {
               
                $.ajax({
                    type: 'post',
                    url: /****/'?page=_offeraction&action=check_showphone',
                    data: {offer_id:offer_id1,user_id:user_id},
                    success: function (data) {
						//alert(data);
						if (data!=""){
						  $(data).insertBefore($(e).parent());
		...
		function ajaxbalance() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
						 ...
						 $("#offers-balance").html(obj.offers);
		...
		function deleteoffer(offer_id1,e) {
			   ...
			   if (confirm('هل أنت متأكد من حذف هذا العرض ؟')) {
                ...
                $.ajax({
                    type: 'post',
                    url:/****/'?page=_offeraction&action=deleteoffer',
                    data: {offer_id:offer_id1},
                    success: function (data) {
		...
		function resetpassword() {
                ...
                $.ajax({
		   		...
		   		function verifie_offer() {

                $.ajax({
                    type: 'post',
                    url: /****/'?page=_offeraction&action=verifie_offer',
                    data: $('#verifie_offer-form').serialize(),
                    success: function (data) {
						     if (data.indexOf('نجاح')!=-1){
						  //$('#newmessage-response').html(data);
						     window.location.assign('?page=offers');
							 $('#umail').val("");
							 }
							 $('#verifie_offer-response').html(data);
				  }
                });

            }
			
		 function verifie_project() {

                $.ajax({
                    type: 'post',
                    url: /****/'?page=_projectaction&action=verifie_project',
                    data: $('#verifie_project-form').serialize(),
                    success: function (data) {
						     if (data.indexOf('نجاح')!=-1){
						  //$('#newmessage-response').html(data);
						     window.location.assign('?page=projects');
							 ...
							 $('#verifie_project-response').html(data);
				  }
                });
